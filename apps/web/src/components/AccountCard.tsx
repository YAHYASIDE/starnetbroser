...
export function AccountCard({ account, onEdit, onInfo, ledgerEntries, allocations, onLedger, onDeviceStatement, client, onOpenClient }: Props) {
  ...
  // Collapsed by default: only the identity (names/email), the Starlink balance due, the
  // WhatsApp button, and "فتح" show - everything else (status dots, renewal countdown, KIT/SN,
  // the customer ledger, alerts, sync time, معلومات/تعديل) is one tap away behind this arrow.
  const [expanded, setExpanded] = useState(false);
  const collapsedEmail = account.expectedEmail || account.starlinkAccountEmail;
    ...
    <article className="account-card">
      <div className="account-card-client-row">
        ...
        <button
          ...
          aria-label={expanded ? "إخفاء التفاصيل" : "عرض كل التفاصيل"}
          title={expanded ? "إخفاء التفاصيل" : "عرض كل التفاصيل"}
        >
      ...
      <div className="account-card-summary">
        ...
        {collapsedEmail && <p className="account-card-email" dir="ltr">{collapsedEmail}</p>}

        <div className="account-card-summary-row">
          <div className="account-detail-block balance-block compact">
            <span className="account-card-label">الرصيد المستحق لـStarlink</span>
            {balanceIsZero ? (
              <span className="badge badge-green muted">لا يوجد مستحق</span>
            ) : (
              <span className="account-card-value balance-value muted">
                {account.currency}{account.balanceDue || "0"}
              </span>
            )}
          </div>
          <div className="account-card-summary-actions">
            {whatsAppButton}
            <button
